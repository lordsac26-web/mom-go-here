...
function SessionStat({ label, value, tone }) {
  return (
    <div className="text-right">
      ...
      <span className={`inline-flex items-center gap-1 rounded-full border px-3 py-1 text-base font-black ${tone}`}>
...
export default function CoinPusherHUD({ balance, loading, collected, spent }) {
  return (
    <div className="flex items-center justify-between shrink-0 px-1">
      ...
      <SessionStat label="Collected" value={collected} tone="border-green-400/40 bg-green-500/20 text-green-300" />
      <SessionStat label="Spent" value={spent} tone="border-red-400/40 bg-red-500/20 text-red-300" />
    </div>
  );
}