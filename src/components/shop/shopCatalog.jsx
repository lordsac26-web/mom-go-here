/**
 * Shop Catalog — defines all purchasable items.
 * IDs must be stable (used as keys in inventory).
 */

export const BALLOON_SKINS = [
  {
    id: "default",
    label: "Classic Red",
    emoji: "🎈",
    preview: "#ef4444",
    price: 0,
    desc: "The original. Always in style.",
    free: true,
  },
  {
    id: "neon_purple",
    label: "Neon Purple",
    emoji: "🫧",
    preview: "#a855f7",
    price: 300,
    desc: "Electric violet glow on every balloon.",
  },
  {
    id: "golden_hour",
    label: "Golden Hour",
    emoji: "⭐",
    preview: "#eab308",
    price: 500,
    desc: "All balloons shimmer in gold.",
  },
  {
    id: "ocean_breeze",
    label: "Ocean Breeze",
    emoji: "🫐",
    preview: "#38bdf8",
    price: 450,
    desc: "Cool cyan tones across the board.",
  },
  {
    id: "cherry_blossom",
    label: "Cherry Blossom",
    emoji: "🌸",
    preview: "#f472b6",
    price: 600,
    desc: "Soft pink petals raining down.",
  },
  {
    id: "midnight",
    label: "Midnight",
    emoji: "🌙",
    preview: "#1e293b",
    price: 750,
    desc: "Dark, mysterious, impossible to ignore.",
  },
  {
    id: "fire",
    label: "Inferno",
    emoji: "🔥",
    preview: "#f97316",
    price: 900,
    desc: "Red-hot flame color palette.",
  },
  {
    id: "rainbow",
    label: "Rainbow Burst",
    emoji: "🌈",
    preview: "rainbow",
    price: 1200,
    desc: "Each balloon gets a different color!",
    rare: true,
  },
  {
    id: "mint_fresh",
    label: "Mint Fresh",
    emoji: "🍃",
    preview: "#34d399",
    price: 400,
    desc: "Cool, crisp minty green balloons.",
  },
  {
    id: "bubblegum",
    label: "Bubblegum Pop",
    emoji: "🍬",
    preview: "#f9a8d4",
    price: 550,
    desc: "Sweet candy-pink for a playful pop.",
  },
  {
    id: "royal_indigo",
    label: "Royal Indigo",
    emoji: "🔮",
    preview: "#6366f1",
    price: 700,
    desc: "Deep regal indigo with a mystic glow.",
  },
  {
    id: "toxic_lime",
    label: "Toxic Lime",
    emoji: "🧪",
    preview: "#a3e635",
    price: 800,
    desc: "Radioactive lime that dares you to pop it.",
  },
  {
    id: "aurora",
    label: "Aurora",
    emoji: "✨",
    preview: "rainbow",
    price: 1400,
    desc: "Shifting northern-lights colors on every balloon.",
    rare: true,
  },
];

// Wheel themes can optionally include premium AI-generated decorative assets:
//  - rim:  ornate outer ring image (hollow center) layered over the spinning wheel
//  - hub:  center medallion image
//  - backdrop: gradient classes for the wheel container background
// Themes without assets fall back to the classic CSS rim/hub.
export const WHEEL_THEMES = [
  {
    id: "default",
    label: "Classic Casino",
    emoji: "🎡",
    price: 0,
    desc: "The standard spin wheel.",
    free: true,
    colors: ["#ef4444", "#f97316", "#eab308", "#22c55e", "#3b82f6", "#a855f7", "#ec4899", "#14b8a6"],
  },
  {
    id: "gold_vegas",
    label: "Royal Gold",
    emoji: "👑",
    price: 1500,
    desc: "Jewel-encrusted gold rim & diamond hub.",
    colors: ["#b45309", "#d97706", "#f59e0b", "#fbbf24", "#92400e", "#a16207", "#ca8a04", "#854d0e"],
    rim: "https://media.base44.com/images/public/69d2319af097365cbf91e620/6228b5afc_generated_image.png",
    hub: "https://media.base44.com/images/public/69d2319af097365cbf91e620/ee3fc1e81_generated_image.png",
    backdrop: "from-amber-950 via-yellow-900 to-amber-950",
    rare: true,
  },
  {
    id: "cosmic",
    label: "Cosmic Nebula",
    emoji: "🌌",
    price: 1500,
    desc: "Glowing neon energy rim from deep space.",
    colors: ["#4338ca", "#6366f1", "#0891b2", "#06b6d4", "#7c3aed", "#a855f7", "#2563eb", "#3b82f6"],
    rim: "https://media.base44.com/images/public/69d2319af097365cbf91e620/05d063d0a_generated_image.png",
    hub: "https://media.base44.com/images/public/69d2319af097365cbf91e620/6b31ca1c7_generated_image.png",
    backdrop: "from-violet-950 via-indigo-900 to-violet-950",
    rare: true,
  },
  {
    id: "galaxy",
    label: "Galaxy",
    emoji: "🪐",
    price: 400,
    desc: "Deep space colors — stars included.",
    colors: ["#1e1b4b", "#312e81", "#4338ca", "#6366f1", "#818cf8", "#a5b4fc", "#c7d2fe", "#e0e7ff"],
    rim: "https://media.base44.com/images/public/69d2319af097365cbf91e620/042dafe22_generated_image.png",
    hub: "https://media.base44.com/images/public/69d2319af097365cbf91e620/7f230d55c_generated_image.png",
    backdrop: "from-indigo-950 via-blue-950 to-indigo-950",
  },
  {
    id: "tropical",
    label: "Tropical",
    emoji: "🌴",
    price: 350,
    desc: "Bright beach vibes all day.",
    colors: ["#22c55e", "#84cc16", "#eab308", "#f97316", "#ef4444", "#ec4899", "#06b6d4", "#a855f7"],
    rim: "https://media.base44.com/images/public/69d2319af097365cbf91e620/4712c5d25_generated_image.png",
    hub: "https://media.base44.com/images/public/69d2319af097365cbf91e620/955480ad4_generated_image.png",
    backdrop: "from-teal-950 via-emerald-900 to-teal-950",
  },
  {
    id: "sunset",
    label: "Sunset Glow",
    emoji: "🌅",
    price: 500,
    desc: "Warm gradient dusk palette.",
    colors: ["#fef3c7", "#fde68a", "#fbbf24", "#f97316", "#ef4444", "#dc2626", "#b91c1c", "#7f1d1d"],
    rim: "https://media.base44.com/images/public/69d2319af097365cbf91e620/973f87ec6_generated_image.png",
    hub: "https://media.base44.com/images/public/69d2319af097365cbf91e620/9d4c97b15_generated_image.png",
    backdrop: "from-orange-950 via-red-900 to-orange-950",
  },
  {
    id: "emerald",
    label: "Emerald City",
    emoji: "💎",
    price: 600,
    desc: "Rich jewel-tone greens and golds.",
    colors: ["#064e3b", "#065f46", "#047857", "#059669", "#10b981", "#34d399", "#6ee7b7", "#a7f3d0"],
    rim: "https://media.base44.com/images/public/69d2319af097365cbf91e620/a663e008d_generated_image.png",
    hub: "https://media.base44.com/images/public/69d2319af097365cbf91e620/ea389b0a7_generated_image.png",
    backdrop: "from-emerald-950 via-green-900 to-emerald-950",
    rare: true,
  },
  {
    id: "monochrome",
    label: "Monochrome",
    emoji: "🖤",
    price: 700,
    desc: "Sleek black and white precision.",
    colors: ["#0f172a", "#1e293b", "#334155", "#475569", "#64748b", "#94a3b8", "#cbd5e1", "#f1f5f9"],
    rim: "https://media.base44.com/images/public/69d2319af097365cbf91e620/2a86e210c_generated_image.png",
    hub: "https://media.base44.com/images/public/69d2319af097365cbf91e620/2bd73b257_generated_image.png",
    backdrop: "from-slate-950 via-gray-900 to-slate-950",
    rare: true,
  },
  {
    id: "candy_pop",
    label: "Candy Pop",
    emoji: "🍭",
    price: 450,
    desc: "Sugary pastel swirl of sweet colors.",
    colors: ["#f472b6", "#f9a8d4", "#c4b5fd", "#a5b4fc", "#7dd3fc", "#6ee7b7", "#fde047", "#fca5a5"],
    backdrop: "from-pink-950 via-fuchsia-900 to-pink-950",
  },
  {
    id: "volcano",
    label: "Volcano",
    emoji: "🌋",
    price: 650,
    desc: "Molten reds and glowing ember oranges.",
    colors: ["#7f1d1d", "#991b1b", "#dc2626", "#ef4444", "#f97316", "#fb923c", "#fbbf24", "#fde047"],
    backdrop: "from-red-950 via-orange-900 to-red-950",
    rare: true,
  },
  {
    id: "arctic",
    label: "Arctic Frost",
    emoji: "🧊",
    price: 550,
    desc: "Icy blues and frozen whites.",
    colors: ["#0c4a6e", "#0369a1", "#0284c7", "#38bdf8", "#7dd3fc", "#bae6fd", "#e0f2fe", "#f0f9ff"],
    backdrop: "from-cyan-950 via-sky-900 to-cyan-950",
  },
];

export const WHEEL_THEME_MAP = Object.fromEntries(WHEEL_THEMES.map(t => [t.id, t]));

export const DART_POWERUPS = [
  {
    id: "multishot",
    label: "Multi-Shot",
    emoji: "🔱",
    price: 150,
    desc: "Fires 3 darts at once.",
    stackable: true,
    maxOwn: 5,
  },
  {
    id: "mirv",
    label: "MIRV Bomb",
    emoji: "💥",
    price: 200,
    desc: "Explodes into cluster darts mid-flight.",
    stackable: true,
    maxOwn: 5,
  },
  {
    id: "sniper",
    label: "Sniper",
    emoji: "🎯",
    price: 175,
    desc: "Pierces through 5 balloons & obstacles.",
    stackable: true,
    maxOwn: 5,
  },
  {
    id: "freeze",
    label: "Freeze",
    emoji: "❄️",
    price: 225,
    desc: "Freezes all balloons for 3 seconds.",
    stackable: true,
    maxOwn: 5,
  },
  {
    id: "gravity",
    label: "Gravity Bomb",
    emoji: "🌀",
    price: 250,
    desc: "Pulls balloons together then pops them.",
    stackable: true,
    maxOwn: 5,
  },
  {
    id: "zipper",
    label: "The Zipper",
    emoji: "⚡",
    price: 300,
    desc: "Bounces off everything, speeds up on every hit.",
    stackable: true,
    maxOwn: 5,
  },
];